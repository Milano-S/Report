...
import com.example.freedem.databinding.FragmentWeekBinding
...
class WeekFragment : Fragment() {
    ...
    //Week Response
    private lateinit var weekResponse: List<ReportData>
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding = FragmentWeekBinding.bind(view)
        binding.toolbarW.inflateMenu(R.menu.frag_menu)
        binding.toolbarW.setNavigationIcon(R.drawable.arrowback_2)
        binding.toolbarW.setNavigationOnClickListener { _ -> findNavController().popBackStack() }

        getWeekDocs()
        weekResponse = mutableListOf()
    }

    private fun getWeekDocs() {
        ...
        val periodDate = vm.weekDate
        val retrofitData = retrofitBuilder.getReports(
            PeriodData("$periodDate 00:00:00", "2"),
            vm.appTokenKey,
            vm.userTokenKey
        )
            ...
            object : Callback<DocumentResponse> {
                override fun onResponse(
                    ...
                    response.body()?.Reports?.forEach { d ->
                        (weekResponse as MutableList<ReportData>).addAll(mutableListOf(d))
                        vm.weekDocumentListV.addAll(mutableListOf(d))
                    }
                    weekResponse.forEach { item ->
                    ...
                    adapter.setOnReportClickListener(object :
                        ...
                        override fun onReportItemClick(position: Int) {
                            val currentReport = weekResponse[position]
                            val savedDocument =
                            ...
                            if (savedDocument == null) {
                                ...
                                findNavController().navigate(R.id.action_weekFragment_to_reportViewFragment)
                            } else {
                                vm.setCurrentReportV(currentReport)
                    ...
                    adapter.saveReportPdf(object : ReportAdapter.OnSaveItemClick {
                        override fun saveReportPdf(position: Int) {
                            val report = weekResponse[position]
